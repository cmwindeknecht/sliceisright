...
export default function AddUpdateIngredient({ ingredients, setTempIngredients }: AdminMenuItemProps) {
  ...
  useEffect(() => {
    if (selectedIngredientName) {
      const ingredient = ingredients.find(item => item.name?.toString() === selectedIngredientName);
      if (ingredient) {
    ...
    try {
      ...
      if (isUpdateMode) {
        setTempIngredients((prev) =>
          prev.map((item) =>
            item.name?.toString() === selectedIngredientName ? ingredient : item
          )
        );
      } else {
